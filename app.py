...
async def start_game(request: Request):
    ...
    if not player_name:
        return RedirectResponse("/name-input", status_code=303)

    if not db_logic.get_player(player_name):
        db_logic.create_player(player_name)

    sequence, settings = ai_engine.generate_sequence(1)
...
async def game_page(player_name: str):
    ...
    return (
        html.replace("{{ player_name }}", player_name)
            .replace("{{ level }}", str(session["level"]))
            .replace("{{ sequence }}", ",".join(map(str, session["sequence"])))
            .replace("{{ speed }}", str(session["settings"]["display_time"]))
    )
...
async def submit_answer(player_name: str, request: Request):
    ...
    if user_seq == session["sequence"]:
        session["level"] += 1
        sequence, settings = ai_engine.generate_sequence(session["level"])
        session["sequence"] = sequence
        session["settings"] = settings

        praise = ai_engine.ai_praise(session["level"])

        return {
            "success": True,
            "level": session["level"],
            "sequence": ",".join(map(str, sequence)),
            "speed": settings["display_time"],
            "message": praise
        }

    # ❌ WRONG ANSWER
    final_marks = calculate_marks(session["level"])
    db_logic.update_player_score(player_name, final_marks)

    feedback = ai_engine.ai_feedback(user_seq, session["sequence"])

    return {
        "success": False,
        "score": final_marks,
        "correct": " ".join(map(str, session["sequence"])),
        "user": user_input,
        "feedback": feedback
    }
